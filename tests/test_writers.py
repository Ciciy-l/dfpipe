...
class TestCSVWriter(BaseWriterTest):
    ...
    def test_write_csv(self):
        ...
        pd.testing.assert_frame_equal(
            read_data,
            self.test_data,
            check_dtype=False  # CSV写入可能改变数据类型
        )
    ...
    def test_write_with_timestamp(self):
        ...
        # 验证文件名包含时间戳
        csv_files = [f for f in files if f.startswith("test_output_") and f.endswith(".csv")]
...
class TestCSVWriterWithMock(unittest.TestCase):
    ...
    @patch('os.makedirs')
    @patch('pandas.DataFrame.to_csv')
    def test_create_nonexistent_directory_with_mock(self, mock_to_csv, mock_makedirs, mock_exists):
        """使用mock测试创建不存在的目录"""
        # 设置模拟行为
    ...
    def test_timestamp_filename_with_mock(self):
        ...
        with patch('os.path.exists', return_value=True) as mock_exists, \
             ...
             patch('dfpipe.writers.csv_writer.datetime') as mock_datetime:
    ...
    def test_write_with_mock_fixture_style(self, mock_datetime, mock_to_csv, mock_exists):
        ...
        # 创建写入器并写入数据
        writer = CSVWriter(output_dir="/fake/dir", filename="test.csv", use_timestamp=False)
...
class TestCSVWriterPytest:
    ...
    def test_with_mock_fixtures(self, test_df):
        ...
        with patch('os.path.exists', return_value=True) as mock_exists, \
            ...
            # 验证使用了正确的文件名（带时间戳）
            args, kwargs = mock_to_csv.call_args
            expected_path = os.path.join("fixture_dir", f"test_mock_fixture_{timestamp}.csv")
