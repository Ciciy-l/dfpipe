...
class TestLogging(unittest.TestCase):
    ...
    def test_setup_logging_formatters(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            self.assertEqual(file_formatter._fmt, '%(asctime)s - %(name)s - %(levelname)s - %(message)s')
            
            # 验证控制台处理器的格式化器
            mock_stream_handler_instance.setFormatter.assert_called_once()
            console_formatter = mock_stream_handler_instance.setFormatter.call_args[0][0]
            self.assertEqual(console_formatter._fmt, '%(asctime)s - %(levelname)s - %(message)s')
