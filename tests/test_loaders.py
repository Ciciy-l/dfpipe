...
class TestCSVLoader(BaseLoaderTest):
    ...
    def test_load_csv(self):
        ...
        test_data2 = pd.DataFrame(
            {"id": [4, 5, 6], "value": [40, 50, 60], "category": ["D", "E", "F"]}
        )
    ...
    def test_load_nonexistent_directory(self):
        """测试从不存在的目录加载数据"""
        # 创建指向不存在目录的加载器
        loader = CSVLoader(input_dir="/nonexistent/dir", file_pattern="*.csv")
    ...
    def test_load_empty_directory(self):
        ...
        # 创建加载器
        loader = CSVLoader(input_dir=empty_dir, file_pattern="*.csv")
    ...
    def test_load_with_encoding(self):
        ...
        # 创建UTF-8编码的CSV文件
        test_data.to_csv(
            os.path.join(input_dir, "utf8.csv"), index=False, encoding="utf-8"
        )

        # 使用UTF-8编码加载
        loader = CSVLoader(input_dir=input_dir, file_pattern="*.csv", encoding="utf-8")
        result = loader.load()
    ...
    def test_load_with_options(self):
        ...
        # 使用自定义分隔符加载
        loader = CSVLoader(
            input_dir=input_dir, file_pattern="*.csv", sep=";"  # 自定义分隔符参数
        )
        result = loader.load()
    ...
    def test_create_nonexistent_directory(self):
        ...
        # 创建加载器
        loader = CSVLoader(input_dir=nonexistent_dir, file_pattern="*.csv")
...
class TestCSVLoaderWithMock(unittest.TestCase):
    ...
    def test_load_csv_with_mock(self, mock_read_csv, mock_glob, mock_exists):
        ...
        mock_glob.return_value = ["/fake/path/data1.csv", "/fake/path/data2.csv"]
        ...
        # 创建CSV加载器并加载数据
        loader = CSVLoader(
            input_dir="/fake/path", file_pattern="*.csv", encoding="utf-8"
        )
        result = loader.load()
        ...
        self.assertTrue(
            all(
                ...
                for col in ["id", "value", "category", "source_file"]
            )
        )
    ...
    @patch("pandas.read_csv")
    @patch("os.makedirs")
    def test_load_nonexistent_directory_with_mock(
        self, mock_makedirs, mock_read_csv, mock_glob, mock_exists
    ):
        """使用mock测试加载不存在的目录"""
        # 设置模拟行为 - 目录不存在
    ...
    @patch("glob.glob")
    @patch("pandas.read_csv")
    def test_load_no_matching_files_with_mock(
        self, mock_read_csv, mock_glob, mock_exists
    ):
        """使用mock测试没有匹配文件的情况"""
        # 设置模拟行为 - 目录存在但没有匹配文件
        mock_exists.return_value = True
        ...
        # 创建CSV加载器并加载数据
        loader = CSVLoader(input_dir="/fake/path", file_pattern="nonexistent_*.csv")
        result = loader.load()
...
class TestCSVLoaderPytest:
    ...
    def test_load_specific_pattern_with_fixture(
        self, temp_dir, temp_csv_files, test_df
    ):
    ...
    def test_load_with_mock_fixture(self, mock_csv_loader):
        ...
        with patch("glob.glob", return_value=files) as mock_glob, patch(
            "os.path.exists", return_value=True
        ) as mock_exists, patch(
            "pandas.read_csv", side_effect=[df1_with_source, df2_with_source]
        ) as mock_read_csv:
            # 创建加载器并加载数据
            loader = CSVLoader(input_dir="/fake/path", file_pattern="*.csv")
            result = loader.load()
...
class TestCustomLoader(unittest.TestCase):
    ...
    def test_custom_loader(self):
        ...
        loader = self.loader_class(
            data=self.test_data, name="custom_loader", description="自定义数据加载器"
        )
