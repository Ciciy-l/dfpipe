...
class TestComponentRegistry(unittest.TestCase):
    ...
    def test_register_processor(self):
        ...
        self.assertEqual(
            ComponentRegistry._processors["TestProcessor"], self.TestProcessor
        )
    ...
    def test_auto_discover(self):
        ...
        with patch(
            "dfpipe.core.registry.os.path.dirname", return_value="/fake/path"
        ), patch(
            "dfpipe.core.registry.os.path.basename",
            side_effect=lambda x: x.split("/")[-1],
        ), patch(
            "dfpipe.core.registry.os.path.exists", return_value=True
        ), patch(
            "dfpipe.core.registry.os.path.join",
            side_effect=lambda *args: "/".join(args),
        ), patch(
            "dfpipe.core.registry.os.listdir", return_value=["test_loader.py"]
        ), patch(
            "dfpipe.core.registry.importlib.import_module"
        ) as mock_import:
            # 创建真实的测试类
            class TestLoader(DataLoader):
                def load(self):
    ...
    def test_discover_components_directory_not_exists(self):
        ...
        with patch("os.path.exists", return_value=False), patch(
            "dfpipe.core.registry.logger.warning"
        ) as mock_warning:
            # 调用_discover_components
            ComponentRegistry._discover_components(
                "/fake/path", DataLoader, ComponentRegistry.register_loader
            )
    ...
    def test_discover_components_import_error(self):
        ...
        with patch("os.path.exists", return_value=True), patch(
            "os.listdir", return_value=["error_module.py"]
        ), patch(
            "dfpipe.core.registry.importlib.import_module",
            side_effect=ImportError("测试导入错误"),
        ), self.assertLogs(
            logger="DataPipeline.Registry", level="ERROR"
        ) as log_ctx:
            # 调用_discover_components
            ComponentRegistry._discover_components(
                "/fake/path", DataLoader, ComponentRegistry.register_loader
